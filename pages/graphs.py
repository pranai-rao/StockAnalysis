...
stocks = pd.read_csv('pages/stock_list.csv')
...
def update_graph(value, relayout_data):
    ...
    if relayout_data and ('xaxis.range' in relayout_data or 'xaxis.range[0]' in relayout_data):
        x_start, x_end = relayout_data.get('xaxis.range', [None, None])
        if x_start is None or x_end is None:
            x_start = relayout_data.get('xaxis.range[0]', df['Date'].min())
            x_end = relayout_data.get('xaxis.range[1]', df['Date'].max())
    else:
